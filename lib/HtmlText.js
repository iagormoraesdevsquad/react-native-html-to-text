...
import { BLOCK_ELEMENTS, INLINE_ELEMENTS, DEFAULT_STYLES } from './constants';

function isHyperlink(tagName) {
    return tagName === 'a';
}
...
function processNode(node, parentKey, props) {
    ...
    if (isText(nodeName)) {
        const key = `${parentKey}_text`;
        return (
            <Text selectable key={key} style={props.style}>
                {node.value}
            </Text>
        );
    }
    ...
    if (isInlineElement(nodeName)) {
        ...
        node.childNodes.forEach((child, index) => {
            if (isInlineElement(child.nodeName) || isText(child.nodeName)) {
                const isStrong = nodeName.includes('strong');
                const isItalic = nodeName.includes('em');
                // TODO:
                // android isn't adding family bold to certain circumstances 
                children.push(
                    processNode(child, `${key}_${index}`, {
                        ...props,
                        style: {
                            ...props.style,
                            fontFamily:
                                (isStrong && props.fontBold) || (isItalic && props.fontItalic) || props.fontRegular,
                        },
                    })
                );
            } else {
        ...
        return (
            <Text selectable key={key} style={[styleForTag(nodeName), props.style]}>
                {children}
            </Text>
        );
    }
    ...
    if (isBlockElement(nodeName)) {
        ...
        node.childNodes.forEach((childNode, index) => {
            ...
            if (isInlineElement(childNode.nodeName) || isText(childNode.nodeName)) {
                ...
                if (lastInlineNodes.length > 0) {
                    children.push(
                        <Text selectable key={`${key}_${index}_inline`} style={[styleForTag(nodeName), props.style]}>
                            {lastInlineNodes}
                        </Text>
                    );
                    lastInlineNodes = [];
        ...
        if (lastInlineNodes.length > 0) {
            const isCenter = node.attrs.indexOf(node.attrs.find(classEle => classEle.value === 'ql-align-center')) > -1;
            const isRight =
                node.attrs.indexOf(node.attrs.find(classEle => classEle.value.includes('ql-align-right'))) > -1;

            children.push(
                <Text
                    selectable
                    key={`${key}_last_inline`}
                    style={[
                        styleForTag(nodeName),
                        {
                            ...props.style,
                            fontFamily: props.fontRegular,
                            alignSelf: isCenter ? 'center' : isRight ? 'flex-end' : null,
                        },
                    ]}
                >
                    {key.includes('li') && `\u2022   `}
...
export default class HtmlText extends Component {
    ...
    render() {
        ...
        return <View style={this.props.containerStyle}>{children}</View>;
    }
}
